...
Transactions are surprisingly hard to achieve. GNU Guix (and its
sibling Nix) have the right idea: Create a new directory for files and
symlink to that in the final step. Nothing is visible until the
symlink changes. Rolling back is simply replacing a symlink.
...
is independent of configuring emacs.

** Parallelism

Independent modules can be executed in parallel. 

** Fine grained output

Output should be fine grained and easy to control. 
